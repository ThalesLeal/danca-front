    ...
    <div v-if="showModal" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h4>{{ form.id ? 'Editar Categoria' : 'Nova Categoria' }}</h4>
        ...
        <div class="modal-body">
          <form @submit.prevent="saveAndClose">
            <div class="form-group">
              ...
              <input v-model="form.descricao" placeholder="Descrição da categoria" required />
            </div>
    ...
    <div class="list-section">
      ...
      <div class="table-container">
        <table>
          ...
          <tbody>
            <tr v-for="c in categorias.results || categorias" :key="c.id">
              <td>{{ c.id }}</td>
              <td><strong>{{ c.descricao }}</strong></td>
              <td class="text-center">
                <button @click="edit(c)" class="btn-edit" title="Editar">
                  <i class="bi bi-pencil-square"></i>
                </button>
                <button @click="remove(c)" class="btn-delete" title="Excluir">
                  <i class="bi bi-trash3-fill"></i>
                </button>
              </td>
            </tr>
            <tr v-if="(!categorias.results || categorias.results.length === 0) && (!categorias || categorias.length === 0)">
      ...
      <div v-if="categorias.next || categorias.previous" class="pagination">
        <button :disabled="!categorias.previous" @click="go(categorias.previous)" class="btn-page">
          <i class="bi bi-chevron-left"></i> Anterior
        </button>
        <button :disabled="!categorias.next" @click="go(categorias.next)" class="btn-page">
...
async function fetchCategorias(url = null) {
  const { data } = await api.get(url || '/categorias/?ordering=-id')
...
async function save() {
  if (form.id) {
    await api.put(`/categorias/${form.id}/`, { descricao: form.descricao })
  } else {
    await api.post('/categorias/', { descricao: form.descricao })
  }
...
async function remove(c) {
  if (confirm(`Excluir categoria "${c.descricao}"?`)) {
...
async function go(url) {
  if (url) await fetchCategorias(url.replace(import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000/api', ''))
}
...
.close-btn { background: rgba(255, 255, 255, 0.2); border: 2px solid rgba(255, 255, 255, 0.3); font-size: 1.5rem; color: white; cursor: pointer; padding: 0; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 12px; transition: all 0.3s; }
